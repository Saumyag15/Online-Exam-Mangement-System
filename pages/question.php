<?php 
    ...
    //get total time and total no. of questions based on the exam of the student
    $tbl_name_qns='tbl_exam';
    $where_qns="exam_id='$exam'";
    $query_qns=$obj->select_data($tbl_name_qns,$where_qns);
    $res_qns=$obj->execute_query($conn,$query_qns);
    if($res_qns==true)
    {
        $row_qns=$obj->fetch_data($res_qns);
        $exam_type=$row_qns['exam_type'];
        $_SESSION['examName']=$exam_type;
...
<!--Body Starts Here-->
        <div class="main">
            <div class="content">
                ...
                Exam: <span class="heavy"><?php echo $exam_type; ?></span>&nbsp;&nbsp;
                    ...
                    <div class="welcome">
                        ...
                        <?php 
                            ...
                            if($sn<=$total_english)
                            {
                                ...
                                $where="is_active='yes' && category='English' && exam='".$exam."' && question_id NOT IN (".$_SESSION['all_qns'].")";
                            }
                            else
                            {
                                //New Query FOR MATHS
                                $where="is_active='yes' && category='Math' && exam='".$exam."' && question_id NOT IN (".$_SESSION['all_qns'].")";
                            }
                            //Get Maths Questions Only
                            //New Query
                            //$where="is_active='yes' && exam='".$exam."' && question_id NOT IN (".$_SESSION['all_qns'].")";
                            //Old Query
                            //$where="is_active='yes'";
                            $limit=1;
